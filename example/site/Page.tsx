/**
 * The reference-example site page layout.
 *
 * A plausible content page built ENTIRELY from `EditableTarget`s. It hardcodes NO
 * copy — every visible string comes from the adapter's content map (seeded by
 * {@link SeedContent}, replaced live by the admin's `cms/sendElements`). Targets:
 * hero, intro (text), showcase (image card), features (list), and split — which
 * holds a `container` content item that expands into two nested container targets.
 */

import type { ReactNode } from "react";
import { EditableTarget } from "@stardust-cms/iframe-adapter";
import { TARGET_IDS } from "../shared/content-model.js";
